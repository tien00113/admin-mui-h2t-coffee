...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  useEffect(() => {
    if (sidebarOpen) {
      ...
      const animationStep = 10; // milliseconds
      ...
      const animate = () => {
        ...
        sidebarElement.style.transform = `translateX(${(1 - easeProgress) * -100}%)`;
  ...
  return (
    <>
      {<aside
        ref={sidebar}
        className={`absolute left-0 top-0 z-9999 ${sidebarOpen ? 'flex' : 'hidden'} h-screen w-52.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark  ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
